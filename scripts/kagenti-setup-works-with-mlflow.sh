...
# Prometheus/Kiali/Shipwright/MLflow disabled (MLflow deployed independently via
# deploy-mlflow.sh for lifecycle decoupling).
# UI/backend installed by default (use --skip-ui to disable).
#
# Usage:
#   ./scripts/kagenti-setup-works-with-mlflow.sh                              # Auto-clones pinned kagenti ref to ~/.cache/kagenti
#   ./scripts/kagenti-setup-works-with-mlflow.sh --kagenti-repo /path/to/kagenti  # Use local clone
#   ./scripts/kagenti-setup-works-with-mlflow.sh --kagenti-repo https://github.com/org/kagenti.git  # Clone from URL
#   ./scripts/kagenti-setup-works-with-mlflow.sh --kagenti-ref main          # Track upstream main instead of pinned working ref
#   ./scripts/kagenti-setup-works-with-mlflow.sh --realm nerc                 # Custom Keycloak realm (default: kagenti)
#   ./scripts/kagenti-setup-works-with-mlflow.sh --skip-ovn-patch             # Skip OVN gateway patch
#   ./scripts/kagenti-setup-works-with-mlflow.sh --skip-mcp-gateway           # Skip MCP Gateway install
#
# Prerequisites:
...
# Skips MLflow (deployed independently via deploy-mlflow.sh).
...
# Build UI helm flags
# The chart's appVersion often doesn't match available image tags on ghcr.io.
# Pin to a known-working commit tag. Override with --kagenti-ui-tag if needed.
KAGENTI_UI_PINNED_TAG="main-fcb02c5"
KAGENTI_UI_FLAGS=()
if $SKIP_UI; then
  ...
  if [ -z "$KAGENTI_UI_TAG" ]; then
    KAGENTI_UI_TAG="$KAGENTI_UI_PINNED_TAG"
  fi
  log_success "Using UI/backend tag: ${KAGENTI_UI_TAG}"
  KAGENTI_UI_FLAGS+=(--set "ui.frontend.tag=${KAGENTI_UI_TAG}")
  KAGENTI_UI_FLAGS+=(--set "ui.backend.tag=${KAGENTI_UI_TAG}")
fi
...
# namespaces later with setup-kagenti-ns.sh once the real namespaces exist.
_adopt_existing_kagenti_cluster_resources
...
if $KUBECTL get deployment mlflow-deployment -n mlflow &>/dev/null 2>&1; then
  ...
  MLFLOW_SCRIPT="$REPO_ROOT/scripts/deploy-mlflow.sh"
  if [ -f "$MLFLOW_SCRIPT" ]; then
    run_cmd bash "$MLFLOW_SCRIPT"
  else
    log_warn "deploy-mlflow.sh not found — deploy MLflow manually:"
    log_warn "  kubectl apply -k platform/observability/mlflow/"
  fi
fi
echo ""
...
# MLflow URL
MLFLOW_HOST=$($KUBECTL get route mlflow -n mlflow -o jsonpath='{.status.ingress[0].host}' 2>/dev/null || \
  $KUBECTL get route mlflow-route -n mlflow -o jsonpath='{.status.ingress[0].host}' 2>/dev/null || echo "")
if [ -n "$MLFLOW_HOST" ]; then
  log_success "MLflow UI: https://$MLFLOW_HOST"
else
  log_info "MLflow (port-forward): $KUBECTL port-forward svc/mlflow-service 5000:5000 -n mlflow"
fi

# Set up Kagenti realm test user passwords via Keycloak API
_setup_kagenti_test_users() {
  if $DRY_RUN; then return; fi

  local kc_host kc_pass token
  kc_host=$($KUBECTL get route keycloak -n "$KC_NAMESPACE" -o jsonpath='{.spec.host}' 2>/dev/null || echo "")
  if [ -z "$kc_host" ]; then return; fi

  kc_pass=$($KUBECTL get secret keycloak-initial-admin -n "$KC_NAMESPACE" \
    -o jsonpath='{.data.password}' 2>/dev/null | base64 -d)
  if [ -z "$kc_pass" ]; then return; fi

  token=$(curl -sk "https://${kc_host}/realms/master/protocol/openid-connect/token" \
    -d "client_id=admin-cli" -d "username=temp-admin" -d "password=${kc_pass}" \
  ...
  for user in alice bob; do
    local uid
    uid=$(curl -sk "https://${kc_host}/admin/realms/${KC_REALM}/users?username=${user}&exact=true" \
...
" 2>/dev/null)
    if [ -n "$uid" ]; then
      curl -sk -o /dev/null -X PUT \
        "https://${kc_host}/admin/realms/${KC_REALM}/users/${uid}/reset-password" \
        -H "Authorization: Bearer $token" \
        -H "Content-Type: application/json" \
        -d "{\"type\":\"password\",\"value\":\"${user}\",\"temporary\":false}" 2>/dev/null
    fi
  done
  log_success "Kagenti test users: alice/alice, bob/bob (realm: ${KC_REALM})"
}
_setup_kagenti_test_users
