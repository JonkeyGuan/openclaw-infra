...
#   - Generated files go into generated/ directory (gitignored) — NEVER commit!
# ============================================================================
...
if $_ENV_REUSE; then
  ...
  # Default any missing variables to empty (old .env files may lack newer vars)
...
if [ "$A2A_ENABLED" = "true" ]; then
  ...
  # Build peer table for MEMORY.md (injected into workspace after pod starts)
  A2A_PEER_TABLE=""
...
if $KUBECTL rollout status deployment/openclaw -n "$OPENCLAW_NAMESPACE" --timeout=300s 2>/dev/null; then
  ...
  if [ -n "$POD" ]; then
    ...
    # Inject A2A peer table into workspace MEMORY.md (agent can edit this later)
    if [ "$A2A_ENABLED" = "true" ] && [ -n "${A2A_PEER_TABLE:-}" ]; then
      echo "$A2A_PEER_TABLE" | $KUBECTL exec -i -n "$OPENCLAW_NAMESPACE" "$POD" -c gateway -- \
        sh -c "
          MEMFILE='${WORKSPACE}/MEMORY.md'
          # Insert peer rows after the table header line
          python3 -c \"
import sys
...
\" 2>/dev/null || true"
      log_success "A2A peer table seeded in MEMORY.md (${A2A_PEER_NAMESPACES:-self only})"
    fi
